...
export class AircraftFormComponent implements OnInit {
  ...
  constructor(private fb:FormBuilder, private route:ActivatedRoute, private router:Router, private svc:AircraftService){}
  ngOnInit(){ const id=this.route.snapshot.paramMap.get('id'); if(id){ const a=this.svc.getById(id); if(a){ this.isEdit=true; this.form.patchValue({ aircraftID:a.aircraftID, model:a.model, category:a.category, complianceStatus:a.complianceStatus }); this.form.controls.aircraftID.disable(); } } }
  submit(){ if(this.form.invalid) return; const v=this.form.getRawValue() as Aircraft; try{ if(this.isEdit){ this.svc.update(v.aircraftID,{ model:v.model, category:v.category, complianceStatus:v.complianceStatus }); } else { this.svc.add({ ...v, serviceHistory:[] }); } this.router.navigate(['/aircraft']); } catch(e:any){ alert(e.message||'Error'); } }
}
