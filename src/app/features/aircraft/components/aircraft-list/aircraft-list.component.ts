...
export class AircraftListComponent implements OnInit {
  ...
  refresh(){ this.aircraft=this.svc.filter(this.term,this.category||undefined,this.compliance||undefined); }
  clearFilters(){ this.term=''; this.category=''; this.compliance=''; this.refresh(); }
  lastServiceDate(a:Aircraft){ if(!a.serviceHistory?.length) return '—'; const s=[...a.serviceHistory].sort((x,y)=>x.date.localeCompare(y.date)); return s[s.length-1].date; }
  remove(id:string){ if(confirm(`Delete aircraft ${id}?`)) this.svc.remove(id); }
}
