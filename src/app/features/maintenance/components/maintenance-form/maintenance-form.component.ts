...
export class MaintenanceFormComponent implements OnInit {
  ...
  ngOnInit(){ this.aircraftSvc.list().subscribe(()=>{ this.aircraftIDs=this.aircraftSvc.ids(); }); const id=this.route.snapshot.paramMap.get('id'); if(id){ const t=this.svc.getById(id); if(t){ this.isEdit=true; this.form.patchValue({ taskID:t.taskID, aircraftID:t.aircraftID, scheduledDate:t.scheduledDate, status:t.status }); this.form.controls.taskID.disable(); } } }
  submit(){ if(this.form.invalid) return; const v=this.form.getRawValue() as MaintenanceTask; try{ if(this.isEdit){ this.svc.update(v.taskID,{ aircraftID:v.aircraftID, scheduledDate:v.scheduledDate, status:v.status }); } else { this.svc.add(v); } this.router.navigate(['/maintenance']); } catch(e:any){ alert(e.message||'Error'); } }
}
